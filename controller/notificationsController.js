  ...
  try {
    if (!req.headers.authorization) {
     ...
     // console.log("UserGeo: ", result);
      ...
      listPromises.push(eventsFinished.filteredEvents);
     ...
     // var applicationsFinished = getApplicationsAndNews(userId);
      ...
      /*
      eventsFinished.then(filteredEventsSec => {
      ...
      Promise.all(listPromises).then(() => {
        Promise.all(promises).then(() => {
          ...
          for (var x = 0; x < notifications.length; x++) {
            ...
            //console.log(poolEvent.notifyCreatedAt);
            var now = new Date();
            var timeDifference = Math.abs(now.getTime() - poolEvent.notifyCreatedAt.getTime());
